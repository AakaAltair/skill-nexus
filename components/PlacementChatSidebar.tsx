// components/PlacementChatSidebar.tsx
"use client";
...
// --- Props Interface ---
interface PlacementChatSidebarProps {
    driveId: string;           // ID of the placement drive this chat belongs to
    driveTitle: string;        // Title of the drive for the header
    currentUser: User | null;    // Current logged-in user (or null)
    commentsEnabled: boolean;    // Whether commenting is allowed for this drive
    isOpen: boolean;              // Controls if the sidebar is visible
    onClose: () => void;          // Function to call when closing the sidebar (usually toggles parent state)
}

// --- Interface for nested comments structure ---
interface NestedPlacementComment extends PlacementComment {
    replies: NestedPlacementComment[]; // Array to hold nested replies
}

// --- Helper Function to Build Nested Structure from flat comment list ---
// Takes a flat array of comments and organizes them into a tree based on parentId
function buildCommentTree(comments: PlacementComment[]): NestedPlacementComment[] {
    const commentMap: { [id: string]: NestedPlacementComment } = {}; // Map for quick lookup by ID
    const nestedComments: NestedPlacementComment[] = []; // Array for top-level comments

    if (!comments || comments.length === 0) return []; // Return empty if no comments

    // First pass: Create map entries for each comment and initialize replies array
    comments.forEach(comment => {
        if (!comment?.id) { console.warn("Skipping comment without ID:", comment); return; }; // Skip invalid comments
        commentMap[comment.id] = { ...comment, replies: [] };
    });

    // Second pass: Link replies to their parents using the map
    Object.values(commentMap).forEach(comment => {
        if (comment.parentId && commentMap[comment.parentId]) {
            // Add to parent's replies array if not already present (handles potential duplicates)
    ...
    // Sort top-level comments and their replies recursively by creation date (ascending for chat flow)
    const sortByDate = (a: PlacementComment, b: PlacementComment) => new Date(a.createdAt.toString()).getTime() - new Date(b.createdAt.toString()).getTime();
    const sortReplies = (commentNode: NestedPlacementComment) => {
        commentNode.replies.sort(sortByDate);
        commentNode.replies.forEach(sortReplies); // Recursively sort replies of replies
    };
    nestedComments.sort(sortByDate); // Sort top-level comments
    nestedComments.forEach(sortReplies); // Sort all nested replies

    return nestedComments; // Return the final nested structure
}


// --- Recursive Comment Item Component ---
// Renders a single comment and recursively calls itself for replies
interface PlacementCommentItemProps {
    comment: NestedPlacementComment; // The comment data (includes replies)
    currentUser: User | null;      // Current user for context
    driveId: string;              // Drive ID (for potential future actions)
...
const PlacementCommentItem: React.FC<PlacementCommentItemProps> = ({
    comment, currentUser, driveId, level, commentsEnabled,
                 ...
                 {currentUser && commentsEnabled && (
                     <div className="pl-1 flex items-center gap-3">
                         <button
                            ...
                            // Disable if currently posting a reply *to this comment*
                            ...
                            aria-label={isReplyingToThis ? `Cancel reply to ${comment.authorName}` : `Reply to ${comment.authorName}`}
                         >
                            {/* Reply Icon */}
...
// --- Main Placement Chat Sidebar Component ---
const PlacementChatSidebar: React.FC<PlacementChatSidebarProps> = ({
    driveId,
    driveTitle,
    currentUser,
    commentsEnabled,
    ...
    // --- State ---
    const [allComments, setAllComments] = useState<PlacementComment[]>([]); // Holds the flat list from API
    ...
    // --- Fetch ALL Comments when sidebar opens or driveId changes ---
    useEffect(() => {
        // Only fetch when sidebar is open and we have a valid driveId
        if (!isOpen || !driveId) {
            setAllComments([]); // Clear comments if closed or no ID
            return;
        }

        let isMounted = true;
        const fetchComments = async () => {
            console.log(`Fetching comments for placement drive ${driveId}`);
            setIsLoading(true); setError(null); setAllComments([]); // Reset state
            try {
                // Fetch comments from the placement drive-specific API endpoint
                const response = await fetch(`/api/placement/drives/${driveId}/comments`);
                ...
                if (!response.ok) {
                    ...
                    throw new Error(errData.error || `Failed to fetch comments (${response.status})`);
                }
                ...
                if (isMounted) {
                    if (data.comments && Array.isArray(data.comments)) {
                        ...
                        const sorted = data.comments.sort((a: PlacementComment, b: PlacementComment) =>
                ...
                console.error("Error fetching placement comments:", err);
                if (isMounted) setError(err.message || "Could not load comments.");
            } finally {
                if (isMounted) setIsLoading(false); // Stop loading indicator
            }
        };
    ...
    }, [isOpen, driveId]); // Dependencies: Run effect when isOpen or driveId changes
    ...
    useEffect(() => {
        // Scroll smoothly to the end after comments are loaded/updated
    ...
    }, [nestedCommentTree, isOpen, isLoading]); // Dependency on the tree ensures scroll after rebuild
    ...
    useEffect(() => {
     ...
     const refetchComments = useCallback(async () => {
        if (!driveId) return; // Need ID to refetch
        console.log(`Refetching placement comments for ${driveId}...`);
        setError(null); // Clear previous fetch errors
        try {
            const response = await fetch(`/api/placement/drives/${driveId}/comments`); // Fetch from correct endpoint
    ...
    }, [driveId]); // Dependency: only driveId
    ...
    const handlePostMessage = useCallback(async (
        ...
        setIsPostingState(true); // Show loading state
        setErrorState(null); // Clear previous errors for this action
        onSuccess(); // Optimistically clear the relevant input field

        try {
            const idToken = await getIdToken(currentUser, true); // Get fresh token
            // Post to the placement drive-specific comments endpoint
            const response = await fetch(`/api/placement/drives/${driveId}/comments`, {
                method: 'POST',
                headers: { 'Content-Type': 'application/json', 'Authorization': `Bearer ${idToken}` },
                body: JSON.stringify({ text: text.trim(), parentId }) // Send trimmed text and parentId
            });
            ...
            console.log("Placement comment post successful, refetching comments...");
            await refetchComments();

        } catch (err: any) {
            console.error("Error posting placement comment:", err)
            setErrorState(err.message || "Could not send message.");
            // If optimistic update was used, would need rollback logic here
        } finally {
            setIsPostingState(false); // Hide loading state
        }
    }, [currentUser, commentsEnabled, driveId, refetchComments]); // Dependencies
    ...
    const renderComments = useCallback((commentsToRender: NestedPlacementComment[], level = 0): JSX.Element[] => {
        const maxIndentLevel = 4; // Max visual indentation levels
        const currentIndentLevel = Math.min(level, maxIndentLevel);
        // Adjust margin based on level, stopping at maxIndentLevel
        // Using Tailwind JIT classes like ml-[9px], ml-[18px] etc. might be more precise if needed
        const indentationClass = level === 0 ? '' : `ml-${currentIndentLevel * 3}`;

        return commentsToRender.map(comment => (
            <div key={comment.id} className={`w-full ${level > 0 ? `pl-3 border-l-2 border-gray-200/70 ${indentationClass}` : ''}`}>
                {/* Render the individual comment item */}
                <PlacementCommentItem
                    comment={comment} // Pass PlacementComment down
                    currentUser={currentUser}
                    driveId={driveId!} // Pass driveId
                    level={level}
                    commentsEnabled={commentsEnabled}
                    // Pass reply state and handlers down
    ...
    }, [currentUser, driveId, commentsEnabled, replyingTo, isPostingReply, replyText, handlePostReply, handleCancelReply, handleReplyKeyDown, handleSetReplyTarget, replyPostError]); // Dependencies


    // --- Styling Variables ---
    // Adjusted positioning: fixed top-16 (assuming 4rem/64px navbar), calculated height
    // Adjusted max-width for consistency
    const sidebarClasses = `fixed top-16 right-0 h-[calc(100vh-4rem)] bg-white border-l border-gray-200 shadow-xl z-40 transition-transform duration-300 ease-in-out flex flex-col w-full max-w-md sm:max-w-lg md:max-w-xl lg:max-w-xl`; // Adjusted max width slightly
    const inputStyle = "flex-grow bg-white border border-gray-300 rounded-lg px-3 py-2 text-sm placeholder-gray-400 text-gray-900 outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent transition duration-150 resize-none disabled:opacity-60";
    ...
    return (
        ...
        <div className={`${sidebarClasses} ${isOpen ? 'translate-x-0' : 'translate-x-full'}`}>
            ...
            <div className="flex items-center justify-between p-4 border-b border-gray-200 flex-shrink-0 bg-white h-16">
                 <h3 className="text-base font-semibold text-gray-900 truncate pr-2" title={driveTitle}> Drive Q&A / Comments </h3>
                 {/* Close Button */}
            ...
            <div className="flex-grow p-4 space-y-3 overflow-y-auto bg-gray-50/50 pb-4">
                 {/* Loading State */}
                 {isLoading && <p className="text-center text-gray-500 text-sm py-4">Loading Q&A...</p>}
                 {/* Error State */}
                 ...
                 {/* Empty State */}
                 {!isLoading && !error && nestedCommentTree.length === 0 && ( <p className="text-center text-gray-400 text-sm py-6 italic">No questions or comments yet. Ask something!</p> )}
                 {/* Render comment tree */}
            ...
            <div className="p-3 border-t border-gray-200 bg-white flex-shrink-0">
                 ...
                 {commentsEnabled ? (
                    ...
                    <p className="text-center text-xs text-gray-500 py-2 italic">Q&A / Comments are disabled for this drive.</p>
                 )}
                  ...
                  {/* Reply-specific errors are shown inline within the PlacementCommentItem */}
            </div>
        </div>
    );
};

export default PlacementChatSidebar;